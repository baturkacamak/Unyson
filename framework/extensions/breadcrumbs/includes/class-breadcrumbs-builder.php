	...
	class Breadcrumbs_Builder {
		...
		public function __construct( $settings = [] ) {
			...
			if ( isset( $settings['labels'] ) ) {
				$this->settings['labels'] = array_merge( $this->settings['labels'],
				                                         $settings['labels'] );
			}
		}
		...
		private function build_breadcrumbs() {
			...
			if ( is_404() ) {
				...
				$s              = '?s=' . apply_filters( 'fw_ext_breadcrumbs_search_query',
				                                         get_search_query() );
				...
				if ( ! empty( $taxonomies ) ) {
					...
					if ( ! empty( $terms ) ) {
						...
						$term        = $lowest_term[0];
						$return      = array_merge( $return,
						                            array_reverse( $this->get_term_hierarchy( $term->term_id,
						                                                                      $term->taxonomy ) ) );
					}
				}
				...
				$return  = array_merge( $return, array_reverse( $this->get_term_hierarchy( $term_id,
				                                                                           'category' ) ) );
			} elseif ( is_tag() ) {
				...
				$return   = array_merge( $return,
				                         array_reverse( $this->get_term_hierarchy( $term_id,
				                                                                   $taxonomy ) ) );
			} elseif ( is_author() ) {
				$author = [];
				...
				if ( is_day() ) {
					$date['name']      = mysql2date( apply_filters( 'fw_ext_breadcrumbs_date_day_format',
					                                                'd F Y' ),
					                                 $day . '-' . $month . '-' . $year );
					...
					$date['name']      = mysql2date( apply_filters( 'fw_ext_breadcrumbs_date_year_format',
					                                                'Y' ), '01.01.' . $year );
