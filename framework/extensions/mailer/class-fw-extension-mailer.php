...
class FW_Extension_Mailer extends FW_Extension
{
     ...
     */
    public function send($to, $subject, $message, $data = [], $settings = [])
    {
        if (empty($settings)) {
        ...
        if ( ! empty($data['from_name'])) {
            $email->set_from_name($data['from_name']);
        }
        ...
        $result = $send_method->send($email, fw_akg($send_method->get_id(), $settings), $data);
    ...
    public function get_send_methods()
    {
        if (empty($this->send_methods)) {
            ...
            require_once dirname(
                             __FILE__
                         ) . '/includes/classes/class-fw-ext-mailer-send-method.php';

            $this->send_methods = [];
            foreach (apply_filters('fw_ext_mailer_send_methods', []) as $send_method) {
    ...
    protected function _init()
    {
        if (is_admin()) {
            add_action(
                ...
                [$this, '_action_extension_settings_form_render']
            );
        }
    }
}
