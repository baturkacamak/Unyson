...
if ( ! class_exists( 'FW_Option_Type_Form_Builder_Item_Recaptcha' ) ) {
	...
	class FW_Option_Type_Form_Builder_Item_Recaptcha extends FW_Option_Type_Form_Builder_Item {
		...
		public function get_thumbnails() {
			return [
				[
					'html' => '<div class="item-type-icon-title" data-hover-tip="' . __( 'Add a Recaptcha field',
					                                                                     'fw' ) . '">' . '<div class="item-type-icon"><img src="' . esc_attr( $this->get_uri( '/static/images/icon.png' ) ) . '" /></div>' . '<div class="item-type-title">' . __( 'Recaptcha',
					                                                                                                                                                                                                                                               'fw' ) . '</div>' . '</div>',
				],
			];
		}
		...
		public function frontend_render( array $item, $input_value ) {
			...
			wp_enqueue_script( 'frontend-recaptcha',
			                   ...
			                   [ 'g-recaptcha' ], fw_ext( 'forms' )->manifest->get_version(),
			                   true );
			wp_localize_script( 'frontend-recaptcha', 'form_builder_item_recaptcha', [
			...
			return fw_render_view( $this->locate_path( '/views/view.php',
				                       ...
				                       'item'  => $item,
				                       'label' => ( isset( $input_value['label'] ) ) ? $input_value['label'] : __( 'Security Code',
				                                                                                                   'fw' ),
				                       'attr'  => [
