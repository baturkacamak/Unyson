...
if ( ! class_exists( 'FW_Option_Type_Form_Builder' ) ) {
	/**
	...
	class FW_Option_Type_Form_Builder extends FW_Option_Type_Builder {
			...
			foreach ( $items as $item_attributes ) {
				...
				// check if required attribute is set and it is unique
				{
					if ( empty( $fixed_item_attributes['shortcode'] ) || isset( $found_shortcodes[ $fixed_item_attributes['shortcode'] ] ) ) {
						$fixed_item_attributes['shortcode'] = sanitize_key( str_replace( '-', '_',
						                                                                 $item_attributes['type'] ) . '_' . substr( fw_rand_md5(),
						                                                                                                            0,
						                                                                                                            7 ) );
					}
		...
		public function frontend_render( array $items, array $input_values ) {
			return fw_render_view( $this->locate_path( '/views/items.php',
			                                           dirname( __FILE__ ) . '/views/items.php' ), [
		...
		public function frontend_get_value_from_items( array $items, array $input_values ) {
			...
			foreach ( $items as $item ) {
				...
				if ( isset( $values[ $item['shortcode'] ] ) ) {
					trigger_error( 'Form item duplicate shortcode: ' . $item['shortcode'],
					               E_USER_WARNING );
				}

				$label = '';
				if ( ! fw_akg( 'options/label', $item, false ) ) {
					$label = fw_akg( 'options/placeholder', $item, '' ) . ': ';
				}

				$values[ $item['shortcode'] ] = isset( $input_values[ $item['shortcode'] ] ) ? $label . $item_types[ $item['type'] ]->get_value_from_item( $input_values[ $item['shortcode'] ] ) : null;

				if ( isset( $item['_items'] ) ) {
					$sub_values = $this->frontend_get_value_from_items( $item['_items'],
					                                                    $input_values );
		...
		protected function _init() {
			...
			if ( is_admin() && defined( 'DOING_AJAX' ) ) {
				add_filter( 'fw_ext:shortcodes:collect_shortcodes_data', [
					$this,
					'_filter_add_form_builder_items_data',
				] );
			}
		}
		...
		protected function _enqueue_static( $id, $option, $data ) {
			...
			wp_enqueue_style( 'fw-builder-' . $this->get_type(),
			                  fw_get_framework_directory_uri( '/extensions/forms/includes/option-types/' . $this->get_type() . '/static/css/styles.css' ),
			                  [ 'fw' ] );

			wp_enqueue_script( 'fw-builder-' . $this->get_type(),
			                   fw_get_framework_directory_uri( '/extensions/forms/includes/option-types/' . $this->get_type() . '/static/js/helpers.js' ),
			                   [ 'fw' ], false, true );

			wp_localize_script( 'fw-builder-' . $this->get_type(), 'fw_' . str_replace( '-', '_',
			                                                                            $this->get_type() ) . '_item_type_contact_form_data',
			                    [
				                    'contact_form' => fw_ext( 'shortcodes' )->get_shortcode( 'contact_form' )->get_item_data(),
