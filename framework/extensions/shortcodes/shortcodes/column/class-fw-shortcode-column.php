...
class FW_Shortcode_Column extends FW_Shortcode {
	private $restricted_types = [ 'column' ];
	...
	public function _init() {
		add_action( 'fw_option_type_builder:page-builder:register_items',
		            [ $this, '_action_register_builder_item_types' ] );

		add_filter( 'fw_ext:shortcodes:collect_shortcodes_data', [
			$this,
			'_filter_add_column_data',
		] );
	}
	...
	public function get_item_data() {
		...
		$data['tag'] = 'column';
		if ( $options ) {
			$data['default_values'] = fw_get_options_values_from_input( $options, [] );
		}
