...
class FW_Shortcode
{
    ...
    {
        $paths = array_merge($this->rewrite_paths, [$this->path]);
        $uris  = array_merge($this->rewrite_uris, [$this->uri]);
    ...
    {
        foreach (array_merge($this->rewrite_paths, [$this->path]) as $path) {
            $actual_path = $path . $rel_path;
            if (file_exists($actual_path)) {
                return $actual_path;
            }
    ...
    final public function render($atts, $content = null, $tag = '')
    {
        if (empty($atts)) {
            ...
            if (version_compare(fw_ext('shortcodes')->manifest->get_version(), '1.3.0', '>=')) {
                ...
                $atts = fw_ext_shortcodes_decode_attr(
                    $atts,
                    $this->tag,
                    /**
    ...
    protected function _render($atts, $content = null, $tag = '')
    {
        ...
        return $view_extra['before'] . fw_render_view(
                $view_file,
                [
