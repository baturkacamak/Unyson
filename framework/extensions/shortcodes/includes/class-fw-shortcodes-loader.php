...
class _FW_Shortcodes_Loader
{
    ...
    private static $disabled_shortcodes = [];

    private static $extension_shortcodes = [];
        ...
        if (version_compare(fw()->manifest->get_version(), '2.2', '<')) {
            ...
            if ($extension->get_declared_source() === 'framework') {
                ...
                self::load_folder_shortcodes(
                    $ext_name,
                    [
                        'path' => fw_get_template_customizations_directory(
                            '/extensions' . $ext_rel_path . '/shortcodes'
                        ),
                        'uri'  => fw_get_template_customizations_directory_uri(
                            '/extensions' . $ext_rel_path . '/shortcodes'
                        ),
                    ],
                    [
                ...
                self::load_folder_shortcodes(
                    $ext_name,
                    [
                        'path' => fw_get_stylesheet_customizations_directory(
                            '/extensions' . $ext_rel_path . '/shortcodes'
                        ),
                        'uri'  => fw_get_stylesheet_customizations_directory_uri(
                            '/extensions' . $ext_rel_path . '/shortcodes'
                        ),
                    ]
                );
    ...
    private static function load_folder_shortcodes($ext_name, $paths, $rewrites = [])
    {
        ...
        if (isset($rewrites['paths'])) {
            $cleared_rewrite_paths = [];
            $cleared_rewrite_uris  = [];
    ...
    private static function load_shortcode($data)
    {
        ...
        $args               = [
            ...
            'path'          => $path,
            'uri'           => $uri,
            'rewrite_paths' => !empty($data['rewrite_paths']) ? $data['rewrite_paths'] : [],
            'rewrite_uris'  => !empty($data['rewrite_uris']) ? $data['rewrite_uris'] : [],
        ];
        $custom_class_found = false;

        if (isset($data["rewrite_paths"]) && isset($data["rewrite_paths"][0])) {
            $class_file = file_exists(
                $data["rewrite_paths"][0] . "/class-fw-shortcode-{$dir_name}.php"
            ) ? $data["rewrite_paths"][0] . "/class-fw-shortcode-{$dir_name}.php" : $class_file;
        }
